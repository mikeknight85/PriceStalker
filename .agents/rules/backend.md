...
* Migrations are located in `backend/src/migrations/NNN_name.ts` and executed automatically at boot by `backend/src/app/migrateOnBoot.ts`.
* **Idempotency is mandatory**: Always use `IF NOT EXISTS`, `ON CONFLICT DO NOTHING`, and existence checks.
* **Never edit shipped migrations**: Never modify existing migrations or `backend/src/migrations/001_baseline.ts`. Always add a new sequential migration file.
...
* **AI Providers**: Implement `AIProvider` (`backend/src/services/ai/providers/types.ts`) and register in `backend/src/services/ai/client.ts`.
* **Notification Providers**: Register in `backend/src/services/notifications/registry.ts`.
