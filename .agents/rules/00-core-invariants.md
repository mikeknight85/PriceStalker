...
## 1. Zero Emoji in UI
* The UI uses custom SVG icons at `frontend/src/components/Icon/Icon.tsx`.
...
* Always format prices and dates using `frontend/src/utils/format.ts` (`formatPrice`, `formatDate`), which safely default `null` to `undefined`.

## 3. Error Boundaries
* Wrap every top-level page and new tab/feature section in `<ErrorBoundary section="...">` (`frontend/src/components/ErrorBoundary`).
...
* **Alert settings live on `items`** (`target_price`, `price_drop_threshold`, `notify_back_in_stock`). Queries joining product & alert settings must use `ITEM_ALERT_COLUMNS` / `ITEM_ALERT_JOIN` and `withItemAlertSettings` (`backend/src/repositories/item-alert-settings.ts`).
