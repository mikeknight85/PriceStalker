  ...
  try {
    ...
    await client.query(`
      ...
      BEGIN
        ...
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'groq_api_key') THEN
          ALTER TABLE users ADD COLUMN groq_api_key TEXT;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'groq_model') THEN
          ALTER TABLE users ADD COLUMN groq_model TEXT;
        END IF;
