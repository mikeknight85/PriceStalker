...
//
// The openid-client Client is expensive to construct (it hits the provider's
// .well-known endpoint and parses the JWKS), so one Client is cached and
...
import { AuthConfig, OidcClaims } from '../../../models/types';
...
function configCacheKey(cfg: AuthConfig): string {
  // Everything that would change the issued Client. Redirect URI is derived
  // from the environment, but is included so env changes also invalidate.
  return JSON.stringify({
    issuer: cfg.oidc_issuer_url,
    clientId: cfg.oidc_client_id,
    clientSecret: cfg.oidc_client_secret, // rotating the secret must rebuild the client
...
export async function getOidcClient(): Promise<Client> {
  ...
  cachedClient = new issuer.Client({
    ...
    // openid-client wants undefined (not null) for public clients. Public
    // clients are unsupported for now, so a secret is required.
...
// Periodic sweep of expired entries. Single process, so a timer is enough.
...
export async function completeFlow(
  ...
  // userinfo supplies email/name when they are absent from the ID token --
  // Authentik, for one, only puts them there with the right scope mapping.
  ...
  // This toggle lets admins accept claims from IdPs that never emit
  // email_verified (Authentik's default mapping, Keycloak without an explicit
  // mapper). It defaults to true, so strict behaviour is still the default.
  if (cfg.oidc_require_email_verified && !emailVerified) {
    throw new Error(
      'Provider did not assert email_verified. An admin can disable this check in Settings -> Authentication.'
    );
  }
