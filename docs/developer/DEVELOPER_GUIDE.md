...
When changing scraper, product, notification, or admin behavior, check the
corresponding guide and diagram references as part of the change. Run
`make diagrams` after editing a lifecycle DOT source, and use
`git diff --check` before committing documentation changes. Audit files under
`docs/audit/` are investigative material and should be verified against the
current source before being treated as implementation documentation.
...
* **[Scraper Lifecycle](SCRAPER_LIFECYCLE.md)**: In-depth technical breakdown of the seven-stage scrape lifecycle.
* **[Logging](LOGGING.md)**: Configuration reference and developer guide for the multi-target logging system.
